<x-app-layout>
    <div class="container mx-auto p-6">
        ...
        <form method="POST" action="{{ route('annonces.adherent.update', $annonce->id) }}" enctype="multipart/form-data" class="bg-white p-8 shadow-lg rounded-lg">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <select name="category" id="category" required class="form-select mt-1 block w-full">
                    ...
                    <option value="bricolage" {{ (old('category', $annonce->category) == 'bricolage') ? 'selected' : '' }}>Bricolage</option>
                    <option value="electricité" {{ (old('category', $annonce->category) == 'electricité') ? 'selected' : '' }}>Électricité</option>
                    <option value="plomberie" {{ (old('category', $annonce->category) == 'plomberie') ? 'selected' : '' }}>Plomberie</option>
                </select>
            </div>
            ...
            <div class="mb-6">
                ...
                @if($annonce->image)
                    <img src="{{ asset('storage/' . $annonce->image) }}" class="mt-4 max-w-xs rounded-md shadow-sm" alt="Uploaded Image">
                @endif
            </div>
